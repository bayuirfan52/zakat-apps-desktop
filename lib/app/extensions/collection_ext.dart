...
extension IterableSlice<E> on Iterable<E> {
  ...
  List<E> slice(int start, [int end = -1]) {
    ...
    if (startValue < 0) {
      startValue = startValue + list.length;
    }
    if (endValue < 0) {
      endValue = endValue + list.length;
    }

    RangeError.checkValidRange(startValue, endValue, list.length);

    return list.sublist(startValue, endValue + 1);
  }
}
